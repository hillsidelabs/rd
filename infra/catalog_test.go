...
`
	...
	err = os.WriteFile(testFilename, []byte(testYAML), 0644)
	if err != nil {
	...
	defer func() {
		if hasOriginal {
			os.WriteFile(testFilename, originalContent, 0644)
		} else {
	...
	catalog, err := NewCatalogFromFile(testFilename)
	if err != nil {
...
`
	...
	tmpFile := "test_hosts.yml"
	err := os.WriteFile(tmpFile, []byte(testYAML), 0644)
	if err != nil {
		t.Fatalf("Failed to write test hosts.yml: %v", err)
	}
	...
	// Test loading the catalog from our test file
	catalog, err := NewCatalogFromFile(tmpFile)
	if err != nil {
		t.Fatalf("Failed to load catalog: %v", err)
	}
	...
	if len(host.Tags) > 0 && (host.Tags[0] != "test" || host.Tags[1] != "development") {
		t.Errorf("Expected tags ['test', 'development'], got %v", host.Tags)
	}
}
